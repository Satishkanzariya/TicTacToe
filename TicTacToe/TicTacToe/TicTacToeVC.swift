...
class TicTacToeVC: UIViewController {
    ...
    override func viewDidLayoutSubviews() {
        ...
        myLabel.frame = CGRect(x: 20, y: 140, width: view.width - 40, height: 50)
        turnLabel.frame = CGRect(x: 90, y: 140, width: view.width - 40, height: 50)
...
extension TicTacToeVC : UICollectionViewDataSource,UICollectionViewDelegate {
    ...
    private func checkWinner() {
        ...
        if !state.contains(2) {
            let alert = UIAlertController(title: "-> Draw <-", message: "oops!. Try again", preferredStyle: .alert)
    ...
    private func announceWinner(player: String) {
        let alert = UIAlertController(title: "\(player) won", message: "", preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: "Dismiss", style: .cancel, handler: { [weak self] _ in
            self?.resetState()
        }))
        DispatchQueue.main.async {
